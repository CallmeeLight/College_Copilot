...
export default function Dashboard({ classes, assignments, attendance, announcements, settings }: DashboardProps) {
  ...
  return (
    <div>
      ...
      {/* Overview Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-7 mb-10">
        {[
          ...
          <ScrollReveal key={i} delay={40 + i * 50}>
            <div className={`glass-card bg-gradient-to-br ${stat.color} border ${stat.borderColor} h-full`}>
              <div className="flex items-center justify-between mb-2">
                <stat.icon size={18} className={`${stat.textColor} transition-transform duration-200 group-hover:scale-110`} />
          ...
          <ScrollReveal delay={120}>
            <div className="ai-glow-card">
              <div className="flex items-center gap-3 mb-4 relative z-10">
                <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500 to-cyan-500 flex items-center justify-center shadow-[0_0_12px_rgba(99,102,241,0.3)]">
                  <Sparkles size={16} className="text-white ai-sparkle-glow" />
                </div>
                <div>
              ...
              <p className="text-sm text-slate-300 leading-relaxed mb-4 relative z-10">{brief}</p>
              <div className="glass-card !bg-white/3 !p-4 mb-4 relative z-10 hover:!bg-white/5 transition-colors">
          ...
          <ScrollReveal delay={180}>
            <div className="glass-card !p-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <button onClick={() => navigate('/timetable')} className="text-xs text-indigo-400 hover:text-indigo-300 flex items-center gap-1 transition-colors">
                ...
                <div className="space-y-3">
                  {todayClasses.map((cls) => {
                    ...
                    return (
                      <div
                        key={cls.id}
                        className={`flex items-center gap-4 p-3.5 rounded-xl transition-all duration-200 hover:translate-x-1 ${
                          status === 'ongoing' ? 'bg-indigo-500/10 border border-indigo-500/20 shadow-[0_0_15px_rgba(99,102,241,0.12)]' :
                          status === 'completed' ? 'opacity-50 hover:opacity-75' : 'bg-white/3 hover:bg-white/6'
                        }`}
                      >
                        <div className="w-1 h-10 rounded-full flex-shrink-0" style={{ background: cls.color }} />
          ...
          <ScrollReveal delay={240}>
            <div className="glass-card !p-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <button onClick={() => navigate('/assignments')} className="text-xs text-indigo-400 hover:text-indigo-300 flex items-center gap-1 transition-colors">
                  ...
                  {overdue.length > 0 && (
                    <div>
                      ...
                      {overdue.slice(0, 2).map(a => (
                        <div key={a.id} className="flex items-center gap-3 p-3 rounded-lg bg-red-500/8 border border-red-500/15 mb-2 transition-all duration-200 hover:translate-x-1 hover:bg-red-500/12">
                          <div className="flex-1 min-w-0">
                  ...
                  {urgent.length > 0 && (
                    <div>
                      ...
                      {urgent.filter(a => !overdue.includes(a)).slice(0, 2).map(a => (
                        <div key={a.id} className="flex items-center gap-3 p-3 rounded-lg bg-white/3 mb-2 transition-all duration-200 hover:translate-x-1 hover:bg-white/6">
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-white truncate">{a.title}</p>
          ...
          <ScrollReveal delay={150}>
            <div className="glass-card !p-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <button onClick={() => navigate('/attendance')} className="text-xs text-indigo-400 hover:text-indigo-300 flex items-center gap-1 transition-colors">
                    ...
                    <div key={a.id} className="group">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs text-slate-300 flex items-center gap-1.5">
                          <span className="w-2 h-2 rounded-full flex-shrink-0 transition-transform group-hover:scale-125" style={{ background: a.color }} />
                          {a.subject}
                        </span>
                        <span className={`text-xs font-semibold ${isLow ? 'text-red-400' : 'text-green-400'}`}>
                          {pct}%
                          {isLow && <AlertTriangle size={11} className="inline ml-1 text-red-400 animate-warning-pulse" />}
                        </span>
                      </div>
              ...
              {deadlines.length === 0 ? (
                ...
                <div className="space-y-2.5">
                  {deadlines.map(a => {
                    ...
                    const isUrgent = d <= 2;
                    return (
                      <div key={a.id} className="flex items-center gap-3 p-3 rounded-lg bg-white/3 transition-all duration-200 hover:translate-x-1 hover:bg-white/6">
                        <div className={`w-2 h-2 rounded-full flex-shrink-0 ${d < 0 ? 'bg-red-400 deadline-pulse-dot' : isUrgent ? 'bg-amber-400 deadline-pulse-dot' : 'bg-green-400'}`} />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs text-white truncate font-medium">{a.title}</p>
                          <p className="text-[0.65rem] text-slate-500">{a.subject}</p>
                        </div>
                        <span className={`text-[0.6rem] font-medium ${d < 0 ? 'text-red-400' : isUrgent ? 'text-amber-400' : 'text-slate-400'}`}>
              ...
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                  ...
                  {unreadAnnouncements.length > 0 && (
                    <span className="bg-indigo-500/20 text-indigo-400 text-[0.6rem] px-1.5 py-0.5 rounded-full font-medium shadow-[0_0_8px_rgba(99,102,241,0.2)]">
                      {unreadAnnouncements.length} new
                    </span>
                  )}
                </h3>
                <button onClick={() => navigate('/announcements')} className="text-xs text-indigo-400 hover:text-indigo-300 flex items-center gap-1 transition-colors">
              ...
              {announcements.length === 0 ? (
                ...
                <div className="space-y-2.5">
                  {announcements.slice(0, 3).map(a => (
                    <div key={a.id} className={`p-3.5 rounded-lg transition-all duration-200 hover:translate-x-1 ${!a.read ? 'bg-indigo-500/8 border border-indigo-500/15 hover:bg-indigo-500/12' : 'bg-white/3 hover:bg-white/6'}`}>
